...
def plot_decision_boundary_2d(model, X_train, y_train, X_test, y_test, feature_names, h=0.02):
    model.eval()

    # Create a mesh over the two features
    ...
    # Model expects scaled inputs if you trained on scaled data
    grid = np.c_[xx.ravel(), yy.ravel()]
    grid_t = torch.tensor(grid, dtype=torch.float32)

    with torch.no_grad():
        Z = model(grid_t).argmax(dim=1).cpu().numpy()
    ...
    # decision regions
    plt.contourf(xx, yy, Z, alpha=0.25, cmap=plt.cm.Set3)

    # scatter training and test points
    scatter_train = plt.scatter(X_train[:, 0], X_train[:, 1], c=y_train, cmap=plt.cm.Set3, edgecolor='k', label='train', alpha=0.8)
    scatter_test  = plt.scatter(X_test[:, 0], X_test[:, 1],  c=y_test,  cmap=plt.cm.Set3, marker='^', edgecolor='k', label='test', alpha=0.9)
